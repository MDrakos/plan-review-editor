...
- **FM-3 (highest blast radius):** `broadcast()` wrapped each `client.write` in try/catch and drops dead sockets — a socket that died between reaps can no longer crash the whole process (and every session). Presence fans out on every join/leave, so this path is now hot. Test: raw sockets killed mid-broadcast + chat-driven fan-out; assert the server survives and a healthy subscriber keeps a valid roster.
