...
| TM-6 — a prototype's own script can hang the reviewer's tab | Orthogonal to A1's CSP (a CPU-bound loop needs no network access). A7 was to have mitigated the peer-triggered *re*-hang (TM-10), but A7 was removed as inert — see FX-12 — so TM-10 is unmitigated too. Accepted risk per the STRIDE catalog's own disposition; unreachable without a real browser. |
